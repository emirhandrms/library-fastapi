...
from app.crud.book_crud import db_create_book, db_list_books, db_get_book, db_update_book, db_delete_book

def service_create_book(book_create: BookCreate) -> BookRead:
...
def service_list_books() -> List[BookRead]:
    return db_list_books()

def service_get_book(book_id: int) -> BookRead:
    book = db_get_book(book_id)
    if not book:
        raise HTTPException(status_code=404, detail="Book not found")
    return book

def service_update_book(book_id: int, book_update: BookUpdate) -> BookRead:
    book = db_update_book(book_id, book_update)
    if not book:
        raise HTTPException(status_code=404, detail="Book not found")
    return book

def service_delete_book(book_id: int):
